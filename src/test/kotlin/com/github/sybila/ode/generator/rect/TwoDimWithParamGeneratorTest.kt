...
class TwoDimWithParamGeneratorTest {
    ...
    private val fragmentTwo = RectangleOdeModel(OdeModel(listOf(v3, v4), listOf(
            OdeModel.Parameter(name = "p1", range = Pair(0.0, 2.0)),
            OdeModel.Parameter(name = "p2", range = Pair(0.0, 2.0))
    )))
    ...
    private fun verify(model: RectangleOdeModel, transitions: List<Pair<Int, Transition<MutableSet<Rectangle>>>>) {
        model.run {
            for (node in (0 until stateCount)) {
                assertTransitionEquals(node.successors(true), *transitions
                        .filter { it.first == node }.map { it.second }.toTypedArray()
                )
                assertTransitionEquals(node.predecessors(true), *transitions
