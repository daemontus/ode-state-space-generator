...
class KotlinParamsOdeTransitionSystemOneDimTest {
    //dv1 = p(v1/2 + 1) - 1
    ...
    private val fragmentOne = KotlinParamsOdeTransitionSystem(OdeModel(listOf(v1), listOf(
            OdeModel.Parameter("p1", Pair(0.0, 2.0))
    )), true)

    private val fragmentTwo = KotlinParamsOdeTransitionSystem(OdeModel(listOf(v2), listOf(
            OdeModel.Parameter("p2", Pair(-2.0, 2.0))
    )), true)

    private fun KotlinParamsOdeTransitionSystem.checkSuccessors(from: Int, bounds: Map<Int, MutableSet<Rectangle>>) {
    ...
    private fun KotlinParamsOdeTransitionSystem.checkPredecessors(from: Int, bounds: Map<Int, MutableSet<Rectangle>>) {
